...
1. **Hardcoded роли.** Добавление новой роли требует правки 5 скриптов (`setup.sh`, `update.sh`, `scheduler.sh`, `notify.sh`) — даже если структура директории корректна.
...
Текущее решение ниже порога 8. Масштабируемость (5) и Генеративность (5) — критически низкие: пользователь не может добавить свою роль без чтения 5 скриптов.
...
- `docs/LEARNING-PATH.md` — §7.5 «Контракт роли»
