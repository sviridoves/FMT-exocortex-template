...
> 1. `/home/vps/IWE/DS-exocortex/roles/extractor/config/routing.md` — таблицы маршрутизации (Pack'и, типы, директории)
> 2. `/home/vps/IWE/DS-exocortex/roles/extractor/config/feedback-log.md` — лог отклонённых кандидатов (не предлагай аналогичные)
...
**5d. Создай содержимое по шаблону.**
...
### Шаг 5e: Двойной routing — domain vs implementation
...
1. **Тест доменности:** Используется ли это понятие/решение в другом проекте/контексте?
   - **Да (доменное)** → маршрутизируй в Pack (шаги 5a-5d)
   - **Нет (реализационное)** → маршрутизируй в DS docs/ (шаг 5f)
...
### Шаг 5f: Формализация реализационных кандидатов (DS docs/)

> Для кандидатов, прошедших тест как «реализационное» в шаге 5e.
...
5. **Создай содержимое** по шаблону DS (без frontmatter trust/epistemic — это не Pack):
...
5. **Прочитай** `01B-distinctions.md` целевого Pack'а — нет ли конфликта.
...
5. Для defer — запиши в `DS-strategy/inbox/` для следующего цикла.
