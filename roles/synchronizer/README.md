...
5. **Report:** Генерирует ежедневный отчёт здоровья системы
...
| `code-scan.sh` | Сканирует DS-* репо на коммиты за 25ч | scheduler (00:00) |
...
```bash
cd /home/vps/IWE/DS-exocortex/roles/synchronizer
bash install.sh
```
...
| 05:00 | Стратег | morning (Пн=session-prep, Вт-Вс=day-plan) | до 22:00 |
