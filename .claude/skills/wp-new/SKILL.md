...
Найди последний номер РП в `MEMORY.md` → следующий порядковый номер. Только целые числа (75, 75…). Буквенные суффиксы (73a, 73b) запрещены.

## Шаг 3. Проверка бюджета

Прочитай текущий бюджет недели из WeekPlan. Предупреди если превышение.

## Шаг 5. Атомарная запись в 5 мест
...
5. **DS-strategy/docs/Strategy.md** → таблица «РП → Результаты» (только для РП ≥3h, добавить строку с маппингом)
